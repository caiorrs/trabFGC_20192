...
#define DIST_BETWEEN_CUBES 2.0f
...
float camera_front_speed = 2.0f;
float camera_side_speed = 2.0f;
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - Caio R. R. S. - Gabriel S. S.", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 1; i <= mapSize * 2 + 3; ++i)
        {
            ...
            new_block.placeOnScene(g_VirtualScene, -transX, -transZ);
